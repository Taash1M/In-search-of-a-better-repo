...
| **Drawing number ambiguity** | Model returns supplier PN instead of controlling number | Drawings have multiple numbers (FPN, controlling #, supplier #) and the model picks the most prominent one | Add prompt guidance: "Prefer the <ORG>/controlling drawing number over supplier or vendor part numbers" |
