...
description: az role assignment create fails with MissingSubscription on <ORG> AI subscription — always use REST API PUT for role assignments.
type: feedback
---

Use the REST API for Azure RBAC assignments on the <ORG> AI ML Technology subscription (`77a0108c-...`), not `az role assignment create`.
...
1. Resolve email → OID: `az ad user show --id "user@<ORG>.com" --query "id" -o tsv`
