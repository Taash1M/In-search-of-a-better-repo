...
description: git push fails with OpenSSL on large payloads through <ORG> corporate proxy — use schannel SSL backend and bundle workflow
type: feedback
originSessionId: aa210407-a09e-461f-aa2f-83d0e2fa4475
---
Git push to GitHub fails with `ssl/tls alert bad record mac` when pushing large payloads (>50MB) from OneDrive-synced clones through <ORG> corporate proxy.

**Why:** The default OpenSSL SSL backend in Git for Windows doesn't handle large RPC payloads through the <ORG> corporate HTTPS inspection proxy. The Windows-native SChannel backend works because it uses the Windows certificate store and TLS stack.
